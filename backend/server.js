  ...
  try {
    ...
    expressApp.post('/api/admin/config/counties', authenticateToken, requireAdmin, async (req, res) => {
      try {
        ...
        let currentOverrides = config && config.value ? config.value : { ...countyOverrides };
    ...
    /**
     * ==========================================
     * 8. SYSTEM HEALTH CHECK & FALLBACK ROUTES
     * ==========================================
     */
    expressApp.get('/api/health', (req, res) => {
      res.json({
        ...
        database: 'Connected',
        locationsDatasetLoaded: Object.keys(countyOverrides).length > 0
      });
    });
    ...
    expressApp.get('/', (req, res) => {
      res.json({ 
        status: 'Online', 
        message: '🌾 Premium Rice & Grain API Architecture is running seamlessly on Render Cloud.',
        version: '3.5.0-ENTERPRISE-RENDER'
      });
    });
    ...
    expressApp.use((req, res) => {
      res.status(404).json({ error: `API route endpoint ${req.originalUrl} not found.` });
    });

    // Global Server Error Handler Middleware
    expressApp.use((err, req, res, next) => {
      console.error('❌ Global Unhandled Exception Handler:', err.stack || err);
    ...
    server.listen(port, hostname, () => {
      console.log(`\n=============================================================`);
      console.log(`🌾 Mwea Rice Hub Standalone Enterprise API Engine Is Live!`);
      console.log(`📡 Listening on Host: ${hostname} | Port: ${port}`);
      console.log(`🌐 Base Public URL: ${RENDER_BASE_URL}`);
      console.log(`=============================================================\n`);
    });
...
// Execute Server Startup Process
startServer();
