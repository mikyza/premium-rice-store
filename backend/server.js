...
const resend = new Resend(process.env.RESEND_API_KEY || 're_fallback_key_for_boot');
...
async function startServer() {
  try {
    ...
    // Dynamic location loading from kenya_locations.json or kenya_locations_2.json
    let kenyaLogisticsHierarchy = {};
    const primaryLocationPath = path.join(__dirname, 'kenya_locations.json');
    const secondaryLocationPath = path.join(__dirname, 'kenya_locations_2.json');

    if (fs.existsSync(primaryLocationPath)) {
      kenyaLogisticsHierarchy = JSON.parse(fs.readFileSync(primaryLocationPath, 'utf8'));
    } else if (fs.existsSync(secondaryLocationPath)) {
      kenyaLogisticsHierarchy = JSON.parse(fs.readFileSync(secondaryLocationPath, 'utf8'));
    }
